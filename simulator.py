...
UNLOAD_CONCRETE_TIME = 10 # 浇筑混凝土时间 分钟
...
class Environment:
    ...
    def verify_dispatchs(self,dispatchs:List[Dispatch]):
        ...
        if len(true_need_dict) != len(dispatch_dict):
    ...
    def continuous_running(self,dispatchs,external_method:callable=None,reposition_method:callable=None):
        
        dispatch_iter = DispatchIterator(dispatchs)
        
        # v = self.verify_dispatchs(dispatchs)
        
        
        while self.order_iter or self.truck_list or dispatch_iter:
            ...
            # 三种事件中最小的时间
            lt_list = [order_lt,event_lt,dispatch_lt]
            ...
            # 碰到车辆事件了                       
                ...
                if current_truck.state == TruckState.Return:
                    ...
                    # 每一行是一个特征 
                    stations_features = [self.get_action_feature(s,current_project) for s in available_stations]
                    
                    
                    
                    must_reposition_index = self.must_reposition_stations(stations_features)
                    
                    if reposition_method !=None and must_reposition_index:
                        ...
                        # 如果没有重定位方法，保持原样，即哪里来的哪里回
                        if reposition_method == None:
                            ret_sid = current_truck.ret_sid
                        # 否则按照特征选取重定位的station
                        else:
                            
                            ret_index = reposition_method(stations_features)
                            ret_sid = available_stations[ret_index]
    ...
    def train_step(self,dispatch_iter,external_method:callable=None):
        ...
        while self.truck_list or dispatch_iter:
            
            event_lt = self.truck_list.next_event_lt()
            dispatch_lt = dispatch_iter.next_dispatch_lt(self.current_time)                                        
            
            # 三种事件中最小的时间
            lt_list = [event_lt,dispatch_lt]
            index = np.argmin(lt_list)
            lt = lt_list[index]
